...
def get_dataset_and_g(args):
    ...
    train_g_igraph = get_train_g_igraph(data.train_triples, args.num_ent)

    return train_dataset, valid_dataset, test_dataset, train_g_sidir, train_g_bidir, train_g_igraph


def get_train_g_igraph(train_triples, num_ent):
    train_triples = np.array(tuple(train_triples))
    src, tgt, rels = train_triples[:, 0], train_triples[:, 2], train_triples[:, 1]
    ...
    # 改成和train_sampler兼容的格式
    graph = Graph(n=num_ent, edges=edgelist, edge_attrs={'relation': list(rels)}, directed=True)
